...
class FakeNews {
    ...
    CheckComment(socket, comment) {
      ...
      if (article.status == this.roundGoal.stance) {
        ...
        this.gameSocket.emit('game:update', { article: comment, reaction: 'likes', followers: this.followers });
        ...
        if (article.status == 'Negative' && this.roundGoal.stance == 'Positive'){
          ...
          console.log(this.followers);
          this.gameSocket.emit('game:update', { article: comment, reaction: 'angry', followers: this.followers });
          ...
          console.log("neutral");
          this.followers += 1000;
          this.gameSocket.emit('game:update', { article: comment, reaction: 'neutral', followers: this.followers });
