...
class FakeNews {
	...
	Checkpoints() {
		...
		if (_.where(this.feed, { 'correct': false }).length >= this.feed.length/4)
			console.log('you got 1/4 the damn shit wrong already')
		else if (_.where(this.feed, { 'correct': false }).length >= this.feed.length/3)
			console.log('you got 1/3 the damn shit wrong already')
		else if (_.where(this.feed, { 'correct': false }).length >= this.feed.length/2)
