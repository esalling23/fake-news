...
class FakeNews {
	...
	constructor() {
	 ...
	 StartGame(socket, initial) {
		...
		this.currentBuckets = _.sample(_.shuffle(this.currentBuckets), (this.currentProfile.level + 2));
	...
	CheckPost(socket, data) {
		...
		var currentArticle = _.where(this.articlePool, { key: data })[0];
		...
		_.each(this.currentBuckets, (bucket) => {
			this.reactions.push(this.CheckBucket(bucket, currentArticle.lovers, currentArticle.haters));
		});

		console.log(this.reactions);

		let originalFollowers = this.currentProfile.level * 10000;
	...
	CheckBucket(bucket, lovers, haters) {
		...
		if (lover.length > 0) {
			...
			this.followers += this.followers * ( parseInt( bucket.ratio.split(':')[0] )/1000 );
			this.gameSocket.emit('game:update', { followers: this.followers });
			return 'likes';

		} else if (hater.length > 0) {
			console.log(bucket.trait.name + ' hates this');
			this.followers += this.followers * ( parseInt( bucket.ratio.split(':')[1] )/1000 );
			this.gameSocket.emit('game:update', { followers: this.followers });
			return 'angry';
			...
			this.followers += this.followers * (ratio/1000);
			this.gameSocket.emit('game:update', { followers: this.followers });
			return 'mixed';
