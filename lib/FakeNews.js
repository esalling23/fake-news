	 ...
	 StartRound(socket, profile) {
		...
		.exec((err, result) => {
			...
			if (this.currentProfile.level == 1) {
				let followers = _.sample(_.shuffle(this.currentBuckets), 1)[0];
			} else {
				let followers = _.sample(_.shuffle(this.currentBuckets), this.currentProfile.level);
			}
			...
			let data = {
			    currentProfile: this.currentProfile,
			    followers: followers,
			    articles: _.sample(_.shuffle(this.articlePool), 12)
			};
