...
class FakeNews {
	...
	CheckBucket(bucket, article, shared) {
		...
		if (article.fake == true){
			let chance = Math.round(Math.random()*10)%2;
			if (chance == 0) {
				let debunked = false;
			} else {
				let debunked = true;
		...
		if (debunked) {
			this.gameSocket.emit('feed:response', { comment: 'follower', msg: _.shuffle(this.article.debunked)[0] });
		}
	...
	FailProfile() {
	 ...
	 ProfileScore(socket) {
	 	...
	 	let skipped = _.where(this.articlePool, { 'shared': false });

	 	let correct = _.where(this.articlePool, { 'correct': true });
	 	// let incorrect = _.where(this.articlePool, { 'correct':false });
